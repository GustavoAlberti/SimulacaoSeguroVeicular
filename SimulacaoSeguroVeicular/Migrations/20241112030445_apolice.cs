using System;
using Microsoft.EntityFrameworkCore.Migrations;

#nullable disable

namespace SimulacaoSeguroVeicular.Migrations
{
    /// <inheritdoc />
    public partial class apolice : Migration
    {
        /// <inheritdoc />
        protected override void Up(MigrationBuilder migrationBuilder)
        {
            migrationBuilder.CreateTable(
                name: "APOLICES",
                columns: table => new
                {
                    Id = table.Column<int>(type: "int", nullable: false)
                        .Annotation("SqlServer:Identity", "1, 1"),
                    CotacaoId = table.Column<int>(type: "int", nullable: false),
                    MarcaVeiculo = table.Column<string>(type: "nvarchar(50)", maxLength: 50, nullable: false),
                    ModeloVeiculo = table.Column<string>(type: "nvarchar(50)", maxLength: 50, nullable: false),
                    AnoVeiculo = table.Column<int>(type: "int", nullable: false),
                    NomeProprietario = table.Column<string>(type: "nvarchar(100)", maxLength: 100, nullable: false),
                    CpfProprietario = table.Column<string>(type: "nvarchar(11)", maxLength: 11, nullable: false),
                    NomeCondutor = table.Column<string>(type: "nvarchar(100)", maxLength: 100, nullable: false),
                    CpfCondutor = table.Column<string>(type: "nvarchar(11)", maxLength: 11, nullable: false),
                    TipoCobertura = table.Column<string>(type: "nvarchar(50)", maxLength: 50, nullable: false),
                    ValorSeguroTotal = table.Column<decimal>(type: "decimal(18,2)", nullable: false),
                    DataEmissao = table.Column<DateTime>(type: "datetime2", nullable: false),
                    DataVigencia = table.Column<DateTime>(type: "datetime2", nullable: false)
                },
                constraints: table =>
                {
                    table.PrimaryKey("PK_APOLICES", x => x.Id);
                });
        }

        /// <inheritdoc />
        protected override void Down(MigrationBuilder migrationBuilder)
        {
            migrationBuilder.DropTable(
                name: "APOLICES");
        }
    }
}
